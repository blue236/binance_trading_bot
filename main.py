...
def _summary_text(cfg, state, equity_now, base_ccy, now_ts=None):
    ...
    now_obj = now_ts or now_tz(cfg.get("logging", {}).get("tz", "UTC"))
    ...
    return (
        "📌 Ops Summary\n"
        f"Mode: {'PAUSED' if paused else 'RUNNING'} / {runtime_mode} | dry_run={cfg.get('general', {}).get('dry_run')} | approval={'ON' if approval else 'OFF'}\n"
        f"Equity: {float(equity_now):.2f} {base_ccy}"
